...
export default function ProfilePage() {
  ...
  // Extract first and last name from user's displayName if available
  ...
  // Mock order history
  const orderHistory: Array<{id: string, date: string, status: string, total: number, items: number}> = [
              ...
              <CardContent className="grid gap-4">
                {wishlist && wishlist.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {wishlist && wishlist.slice(0, 3).map((item: any) => (
                    ...
                    {wishlist && wishlist.length > 3 && (
                      <Link href="/wishlist" className="border rounded-lg p-4 flex flex-col items-center justify-center h-full">
                        <div className="text-xl font-semibold mb-2">
                          +{wishlist.length - 3} more {wishlist.length - 3 === 1 ? 'item' : 'items'}
                        </div>
