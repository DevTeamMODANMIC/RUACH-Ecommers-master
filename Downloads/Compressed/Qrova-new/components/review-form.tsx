...
export function ReviewForm({ productId, productName, availableCountries, onClose, onSubmit }: ReviewFormProps) {
  ...
  const { user, profile } = useAuth() // Get both user and profile
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const reviewData = {
        ...
        userName: profile?.name || user?.email || "Anonymous", // Use profile name, then email
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="verified" className="text-sm">
                  I purchased this product from Grova
                </Label>
              </div>
