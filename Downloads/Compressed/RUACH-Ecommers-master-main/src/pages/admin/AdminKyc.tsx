...
export default function AdminKyc() {
  ...
  const handleResetKyc = async () => {
    ...
    const confirmed = window.confirm(
      "Are you sure you want to reset this user's KYC? They will need to complete the verification process again."
    );
    if (!confirmed) return;
    ...
    try {
      ...
      const vendorQuery = query(
        ...
        where("ownerId", "==", selectedRecord.id)
      );
      const vendorSnapshot = await getDocs(vendorQuery);

      const vendorUpdatePromises = vendorSnapshot.docs.map((vendorDoc) =>
        updateDoc(doc(db, "vendors", vendorDoc.id), {
          kycStatus: "not-started",
          kycData: null,
        })
      );

      if (vendorUpdatePromises.length > 0) {
      ...
      toast({
        title: "KYC Reset",
        description: "The user will need to redo their KYC verification.",
      });
    } catch (error: any) {
      console.error("Error resetting KYC:", error);
      toast({
        title: "Error",
        description: error.message || "Failed to reset KYC. Please try again.",
        variant: "destructive",
      });
    } finally {
        ...
        <DialogContent className="max-w-2xl">
          ...
          <DialogFooter>
            <Button
              ...
              disabled={isResetting || detailLoading}
            >
              {isResetting ? "Resetting..." : "Reset KYC"}
            </Button>
